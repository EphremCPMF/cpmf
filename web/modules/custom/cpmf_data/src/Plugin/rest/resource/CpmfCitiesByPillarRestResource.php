 ...
 *
 * @RestResource(
 *   id = "cpmf_cities_by_pillar_rest_resource",
 *   label = @Translation("Cpmf cities by pillar rest resource"),
 *   uri_paths = {
 *     "canonical" = "/rest/cities-by-pillar/{pillar}"
 *   }
 * )
 */
class CpmfCitiesByPillarRestResource extends ResourceBase {
  ...
  /**
   * Constructs a new CpmfCitiesByPillarRestResource object.
   *
    ...
    if ($pillar) {
      ...
      foreach ($cities as $nid => $city) {
        ...
        if ($indicator_data) {
          ...
          $pillar_score = round($indicator_data->get($pillar_field)->value);

          $city_values[] = [
            'title' => $city->label(),
            'pillar_color' => $pillar_nodes[$pillar_field]->field_color->color,
            'pillar_value' => $pillar_score
          ];
        }
      }
