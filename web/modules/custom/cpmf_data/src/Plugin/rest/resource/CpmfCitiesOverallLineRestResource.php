 ...
 *
 * @RestResource(
 *   id = "cpmf_cities_overall_line_rest_resource",
 *   label = @Translation("Cpmf cities overall line rest resource"),
 *   uri_paths = {
 *     "canonical" = "/rest/cities-overall-line"
 *   }
 * )
 */
class CpmfCitiesOverallLineRestResource extends ResourceBase {
        ...
        foreach ($years as $year_name => $year_tid) {
          ...
          foreach($indicator_data_nodes as $indicator_data) {
            ...
            if ($indicator_data->field_year->getValue()[0]['target_id'] == $year_tid) {
              $score[$year_name] = round(cpmf_data_get_overall_score($indicator_data));
            }
          }
